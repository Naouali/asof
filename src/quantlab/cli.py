...
skips a failed ingest, or returns a plausible-looking zero.
"""
...
app = typer.Typer(
    name="quantlab",
    help="Multi-asset market data ETL: fetch from free sources into a parquet lake.",
...
def data_catalogue(
    ...
    if source is not None:
        ...
        if not spec.point_in_time_safe:
            console.print(
                "  [yellow]restated data -- its history is not what was known then[/yellow]"
            )
        console.print("\n  [bold]caveats[/bold]")
...
def serve(
    ...
    """Serve the web app: the read-only API and the built interface.

    There is no login yet, so the default is this machine only. Listening on any
    other address makes every disclosure in the lake readable by whoever can reach
    it; that is allowed, because a container has to, and it is said out loud.
    """
    import uvicorn
    ...
    if host not in _LOOPBACK:
        err_console.print(
            ...
            "anyone who can reach this address can read everything in the lake. Put it "
            "behind something that authenticates, or publish the port to 127.0.0.1 only."
        )
    console.print(f"asof is at [bold]http://{host}:{port}[/bold]  (API reference: /api/docs)")
    uvicorn.run(
        create_app(settings, warm=True), host=host, port=port, log_level=settings.log_level.lower()
    )
...
def worker_run(
    ...
    The worker exists so that ad-hoc jobs (`make shell`, `make ingest`) have a
